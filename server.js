...
/*  "/api/items"
 *    GET: finds all items for sale
 *    POST: creates a new item for sale
 */
...
app.get("/api/items", function(req, res) {
  db.collection(TEXTBOOK_COLLECTION).find({}).toArray(function(err, docs) {
    if (err) {
...
app.post("/api/items", function(req, res) {
  ...
  if (!req.body.name) {
    ...
    db.collection(TEXTBOOK_COLLECTION).insertOne(newItem, function(err, doc) {
      if (err) {
